🧊 TecnoChiller — Site Institucional

Site institucional para empresa especializada em refrigeração industrial, desenvolvido com HTML, CSS e JavaScript puro e hospedado na Netlify.

🔗 Acesse o site ao vivo →

📋 Sobre o Projeto
A TecnoChiller é uma empresa de Franco da Rocha — SP, especializada em soluções de refrigeração industrial. Este repositório contém o site institucional da empresa, criado para apresentar seus serviços, facilitar o contato com clientes e gerar novos orçamentos via WhatsApp.
Serviços apresentados no site

🔧 Manutenção — Preventiva e corretiva em chillers
⚙️ Instalação — Sistemas completos de refrigeração industrial
📐 Projetos — Desenvolvimento de soluções sob medida


🚀 Tecnologias
Este projeto foi construído com tecnologias web nativas, sem dependência de frameworks:
Tecnologia Uso HTML5 Estrutura e marcação semântica CSS3 Estilização, layout e responsividade JavaScript Interatividade e comportamento dinâmico Netlify Hospedagem e deploy contínuo

📁 Estrutura do Projeto
TecnoChiller-site/
│
├── index.html              # Página principal
│
├── assets/
│   └── img/               # Imagens e ícones
│
├── css/                   # Folhas de estilo
│
├── js/                    # Scripts JavaScript
│
├── components/            # Componentes reutilizáveis (header, footer, etc.)
│
└── pages/                 # Páginas internas (ex: contato)

🖥️ Como Rodar Localmente
Não há dependências ou build necessário — basta clonar e abrir no navegador.
bash# 1. Clone o repositório
git clone https://github.com/luizsansilv/TecnoChiller-site.git

# 2. Acesse a pasta
cd TecnoChiller-site

# 3. Abra o arquivo index.html no seu navegador
# (ou use a extensão Live Server no VS Code para hot-reload)

💡 Dica: Use a extensão Live Server no VS Code para uma experiência de desenvolvimento com auto-reload.


🌐 Deploy
O site está hospedado na Netlify com deploy automático a partir da branch main.
Qualquer push para a branch principal atualiza o site em produção automaticamente.
URL de produção: https://tecchiller.netlify.app/

📞 Contato da Empresa
CanalInformação📞 Telefone/WhatsApp(11) 97306-6874📧 E-mailcontato@tecnochiller.com.br📍 LocalizaçãoFranco da Rocha — SP

👨‍💻 Desenvolvedor
Desenvolvido por @luizsansilv.

📄 Licença
Este projeto é de uso privado e pertence à empresa TecnoChiller. Todos os direitos reservados.
